...
# Graph JSON path
GRAPH_JSON_PATH = os.path.join(os.path.dirname(__file__), "..", "graph-visualize", "graph.json")
...
# =============================================================
def invoke_llm_tracked(prompt_template, llm, inputs: dict, state: dict, node_name: str = "") -> str:
    """
    ...
    usage = state.get("token_usage", {"input": 0, "output": 0, "total": 0, "calls": 0, "details": []})
    if hasattr(ai_message, 'usage_metadata') and ai_message.usage_metadata:
        ...
        usage["calls"] += 1
        usage["details"].append({"node": node_name, "input_tokens": 0, "output_tokens": 0, "total": 0})
...
""")
    ...
    try:
        raw_out = invoke_llm_tracked(prompt, llm, {"question": question}, state, "Intent Analyzer")
...
def semantic_router_node(state: AgentState) -> dict:
    ...
    # Chọn route
    scores = {"sql": sql_max, "vector": vec_max, "graph": graph_max, "hybrid": hybrid_max}
    ...
    if route != "hybrid":
        ...
        entity_signals = any([
            ...
            any(kw in q_lower for kw in [
                "tracking", "kiện hàng", "lô hàng", "đơn hàng",
                "sản phẩm", "kho", "bị hư", "bị hỏng", "bị bẹp",
                "bị vỡ", "bị ẩm"
            ])
        ])
        policy_signals = any(kw in q_lower for kw in [
            "quy định", "chính sách", "hướng dẫn", "quy trình",
            "luật", "thủ tục", "bảng giá", "điều khoản",
            "đóng gói", "bảo hiểm", "bồi thường", "khiếu nại",
            "xử lý", "phí", "cước"
        ])
    ...
    if needs_calc and recommended_route == "sql":
        logs.append("⚠️ **Route Override**: Đã bẻ lái sang SQL vì nhận diện được câu hỏi Thống kê toàn cục.")

    if needs_calc:
        logs.append("🔢 **Smart Calculator**: LLM phát hiện câu hỏi cần thống kê tính toán")
    if target_collections:
        logs.append(f"🔍 **Collections Detected**: {', '.join(target_collections)}")

    return {"route": route, "needs_calculation": needs_calc, "target_collections": target_collections, "logs": logs}
...
""")
    ...
    llm = get_generator_llm()
    raw_sql = invoke_llm_tracked(sql_prompt, llm, {"table_info": table_info, "question": q}, state, "SQL Generator")
...
def vector_retriever_node(state: AgentState) -> dict:
    ...
    if collection_filter:
        logs.append(f"🔍 **Vector Filter**: LLM chỉ định tìm trong collections `{', '.join(collection_filter)}`")
    ...
    if top_results:
        ...
        logs.append(f"✅ **Vector Result**: Tìm thấy {len(top_results)} tài liệu (Top similarity: {top_sim})")
        if len(queries) > 1:
...
""")
        llm = get_generator_llm()
        entities_raw = invoke_llm_tracked(extract_prompt, llm, {"question": q}, state, "Graph Entity Extractor")
        entities = [e.strip().replace(" ", "_") for e in entities_raw.strip().split("\n") if e.strip()]
    ...
    for node in matched_nodes[:10]:
        node_id = node["id"]
        node_type = node["type"]
        score_info = f" (sim: {node.get('_score', '-'):.4f})" if "_score" in node else ""
    ...
    if context_parts:
        context = "\n".join(context_parts)
        logs.append(f"✅ **Graph Result**: {len(matched_nodes)} nodes, {related_edges_count} relationships")
    else:
...
def hybrid_retriever_node(state: AgentState) -> dict:
    ...
    """
    logs = state.get("logs", [])
    logs.append("🔀 **Hybrid Retriever**: Đang query cả VectorDB lẫn Knowledge Graph...")
    ...
    if merged_parts:
        ...
        context = "Không tìm thấy thông tin liên quan trong cả VectorDB lẫn Knowledge Graph."
...
def calculator_node(state: AgentState) -> dict:
    ...
    logs.append("🔢 **Data Analyst Node**: Đang phân tích ngữ nghĩa và tính toán số liệu...")
...
""")
    ...
    llm = get_generator_llm()
    calc_output = invoke_llm_tracked(calc_prompt, llm, {"context": context, "question": q, "route": route}, state, "Smart Calculator")

    logs.append("✅ **Data Analyst Node**: Bảng / Báo cáo tính toán nội suy đã sinh xong.")
...
""")
    ...
    llm = get_generator_llm()
    answer = invoke_llm_tracked(gen_prompt, llm, {"context": context, "question": q, "route": route}, state, "Generator")
...
""")
    ...
    llm = get_grader_llm()
    grade = invoke_llm_tracked(grader_prompt, llm, {
        "context": state.get("context", ""),
        "answer": state.get("answer", ""),
    }, state, "Hallucination Grader").strip().lower()
    ...
    if grade == "pass":
        ...
        logs.append(f"⚠️ **Grader**: Phát hiện ảo giác! (FAIL) — Retry count: {state.get('retry_count', 0)}")
...
def synonym_rewriter_node(state: AgentState) -> dict:
    ...
    if len(variants) > 1:
        ...
        logs.append("⚠️ **Synonym Rewriter**: Không tìm thấy từ đồng nghĩa, dùng LLM rewrite nhẹ...")
...
""")
        ...
        rewritten = invoke_llm_tracked(rewrite_prompt, llm, {"question": q}, state, "Synonym Rewriter").strip()
...
""")
    ...
    llm = get_rewriter_llm()
    rewritten = invoke_llm_tracked(rewrite_prompt, llm, {"question": state.get("rewritten_question") or state["question"]}, state, "LLM Deep Rewriter")
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("**📁 Dataset:**")
        st.markdown("- 1,100 files (500 phiếu + 398 manifest + 120 biên bản + 60 tờ khai + 20 policy + 2 CSV)")
        st.markdown("---")
